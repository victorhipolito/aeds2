...
class Modelagem{
	...
	public static void selecaoParcial(Restaurante[] r, int k, int t){
		try{
			File arquivo = new File("./902693_selecao_parcial.txt");
			arquivo.createNewFile();
			int comp, mov, menor;
			...
			for (int i = 0; i < k; i++){
				menor = i;
				for (int j = i+1; j < t; j++){
					String nomeAtual = r[j].getNome();
					if (nomeAtual.compareTo(r[menor].getNome()) < 0) {menor = j;}
					comp++;
				}
				//System.out.println(restaurantes[menor].getNome());
				swap(r, i, menor);
