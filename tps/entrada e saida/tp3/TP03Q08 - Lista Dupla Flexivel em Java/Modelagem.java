...
class ListaDupla{
	...
	public void inserirInicio(Restaurante r){
		...
		if(tam == 0){
			ini = fim = nova;
			ini.prox = ini.ant = fim;
			fim.prox = fim.ant = ini;
		}
		else{
			nova.prox = ini;
			ini.ant = nova;
	...
	public void inserir(Restaurante r, int pos){
		if(pos == 0){inserirInicio(r);}
		else if (pos == tam){inserirFim(r);}
		else{
			Celula nova = new Celula(r);
			Celula tmp = ini;
			for(int i = 1; i < pos; i++){tmp = tmp.prox;}
			nova.prox = tmp.prox;
			tmp.prox.ant = nova;
			nova.ant = tmp;
			tmp.prox = nova;
	...
	public Restaurante remover(int pos){
		Restaurante retorno;
		if(pos == 0){retorno = removerInicio();}
		else if(pos == tam-1){retorno = removerFim();}
		else{
			Celula aux = ini;
			for(int i = 1; i < pos; i++){aux = aux.prox;}
			retorno = aux.prox.elem;
...
class Modelagem{
	...
	public static void main(String args[]){
		...
		String[] selecao;
		Restaurante retorno;
		int posRest, posLista;
		selecao = sc.nextLine().split(" ");
		for(int i = 0; i < num; i++){
			selecao = sc.nextLine().split(" ");

			//Inserir
			if(selecao[0].equals("I*")){
				posLista = Integer.parseInt(selecao[1]);
				posRest = Integer.parseInt(selecao[2]);
				lista.inserir(c[posRest], posLista);
			}
			//Remover
			else if (selecao[0].equals("R*")){
				posLista = Integer.parseInt(selecao[1]);
				retorno = lista.remover(posLista);
				System.out.println("(R)"+retorno.getNome());
			}
			//inserirInicio
			else if (selecao[0].equals("II")){
				posRest = Integer.parseInt(selecao[1]);
				lista.inserirInicio(c[posRest]);
			}
			//removerInicio
			else if (selecao[0].equals("RI")){
				retorno = lista.removerInicio();
				System.out.println("(R)"+retorno.getNome());
			}
			//inserirFim
			else if (selecao[0].equals("IF")){
				posRest = Integer.parseInt(selecao[1]);
				lista.inserirFim(c[posRest]);
			}
			//removerFim
			else if (selecao[0].equals("RF")){
		...
		while(lista.tam != 0){
			retorno = lista.removerInicio();
			System.out.println(retorno.formatar());
		}
