...
void inserir(Fila* fila, Rest* r){
	if (fila->ini != NULL){
		Cel* tmp = fila->ini;
		while(tmp->prox != NULL){tmp = tmp->prox;}
		tmp->prox = new_cel(r);
	}
	else{fila->ini = new_cel(r);}
}

Rest* remover(Fila* fila){
	Rest* resp = fila->ini->elem;
	Cel* aux = fila->ini;
...
int main(){
	...
	scanf("%d", &i);
	while(i != -1){
		inserir(fila, colecao->restaurantes[find_rest_by_id(colecao, i)]);
		scanf("%d", &i);
	}
	...
	for(int i = 0; i < reg; i++){
			...
			switch(comando){
				...
				case 'I':
					scanf(" %d", &pos_rest);
					inserir(fila, colecao->restaurantes[find_rest_by_id(colecao, pos_rest)]);
					break;
				// Caso remover
				case 'R':
					retorno = remover(fila);
					printf("(R)%s\n", retorno->nome);
					break;
				}
			}
	...
	while(fila->ini != NULL){
		retorno = remover(fila);
		formatar_restaurante(retorno, buffer);
		printf("%s\n", buffer);
	}
